...
(function () {
  ...
  function openMunicipioModal(r) {
    ...
    document.getElementById("munRepPdfBtn").addEventListener("click", () => {
      ...
      const old = document.getElementById("munPdfContainer");
      if (old) old.remove();

      // Monta o HTML completo inline (sem depender de elementos do modal)
      const bodyHtml = document.getElementById("munRepBody").innerHTML;

      const container = document.createElement("div");
      container.id = "munPdfContainer";
      container.style.cssText = "position:fixed;top:0;left:0;width:794px;background:#fff;font-family:Inter,sans-serif;color:#1d1d1f;z-index:99999;visibility:hidden;";
      container.innerHTML = `
      ...
      const opt = {
        ...
        image: { type: "jpeg", quality: 0.97 },
        html2canvas: { scale: 2, useCORS: true, scrollX: 0, scrollY: 0, backgroundColor: "#ffffff", windowWidth: 794 },
        jsPDF: { unit: "mm", format: "a4", orientation: "portrait" },
        pagebreak: { mode: ["css", "legacy"] }
      };

      // Deixa o browser renderizar o container antes de capturar
